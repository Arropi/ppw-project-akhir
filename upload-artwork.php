...
<body class=" vh-100">
    ...
    <main class="px-5 py-3 mt-4 h-75 w-100 gap-2">
        <h1>Artwork</h1>
        <form action="" method="post" class="h-50 d-flex w-100 gap-3" enctype="multipart/form-data">
                    ...
                    <?php
                        if(isset($_POST['simpan'])){
                            ...
                            if(empty($errors)){
                                ...
                                $query_insert = "INSERT INTO artwork (user_id, judul, kategori, deskripsi, gambar, tipe_gambar) VALUES ($user_id_from_session, '$escaped_title', '$escaped_kategori','$escaped_description', '$image_data', '$image_type')";
                                $insert_data = mysqli_query($koneksi, $query_insert);
                                ?> 
                                <a href="profile.php" class=" text-decoration-none">Kembali ke halaman</a>
                                <?php
                            } else {   
            ...
            <div class="upload-desc d-flex flex-column justify-content-between">
                <div>
                    ...
                    <input type="text" name="title" id="username" class="form-control w-50 form-input-user" placeholder="Masukkan username baru" required>
                    <label for="bio" class="form-label mt-3"><h2>Description</h2></label>
                    <textarea type="text" name="description" id="bio"  placeholder="Masukkan bio baru" class=" form-control h-75 w-100 form-input-bio" style="resize: none;"></textarea>
                </div>
                <div class="d-flex justify-content-between mt-2">
                    <div class="d-flex justify-content-center align-items-center h-100 w-auto gap-4 text-center">
                        <h3 class="m-0">Kategori</h3>
                        <div class="d-flex justify-content-center align-items-center h-100 gap-2 w-auto">
                            <input class="form-check-input m-0" type="radio" name="kategori" checked value="Digital">
                            <label class="form-check-label" for="radioDefault1">
                                Digital
                            </label>
                        </div>
                        <div class="d-flex justify-content-center align-items-center h-100 gap-2 w-auto">
                            <input class="form-check-input m-0" type="radio" name="kategori" value="Non Digital">
                            <label class="form-check-label" for="radioDefault2">
                                Non Digital
                            </label>
                        </div>
                        <a href="upload-lelang.php">
                            <input type="button" value="Lelang"  class="btn btn-light btn-lelang shadow" >
                        </a>
                    </div>
                </div>
            </div>
